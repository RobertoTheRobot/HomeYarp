...
- `Components/App.razor` — root document. `<Routes @rendermode="InteractiveServer" />` makes the whole router globally interactive. Loads MudBlazor 9 + BlazorMonaco assets; **script order is load-bearing**: `_content/BlazorMonaco/jsInterop.js` (sets `var require = { paths }` at the top of its module — that's what Monaco's AMD loader picks up) → `loader.js` → `editor.main.js` → `_content/MudBlazor/MudBlazor.min.js` → **`_framework/blazor.web.js` LAST**. If `blazor.web.js` runs first, the circuit prerenders `JsonEditor` while `window.monaco` is still undefined, BlazorMonaco throws, and the circuit terminates on connect — page renders but inputs never gain focus and typing is invisible. See `tasks/lessons.md` "App.razor script order".
- `Components/Routes.razor` — router with `MainLayout` as default.
- `Components/Layout/{MainLayout,NavMenu}.razor` — shell + sidebar. `MainLayout` hosts the four MudBlazor providers (`MudThemeProvider IsDarkMode="true"`, `MudPopoverProvider`, `MudDialogProvider`, `MudSnackbarProvider`).
...
- `Components/Shared/TrustCertificateDialog.razor` — reusable MudDialog with platform-specific snippets (Windows / Debian-Ubuntu / RHEL-Fedora / macOS Keychain / Docker daemon / Docker Desktop / Firefox) for trusting a downloaded cert. Takes `CertSlug` + `PrimaryHost` parameters; opened from the per-row "How to trust this cert" icon on `Certificates.razor` and `Applications.razor` (only shown for self-signed certs — Manual / ACME-issued certs don't need a trust step).
- `Components/Shared/ProgressLog.razor` — append-only log of step messages from an `IProgress<string>`, used by long-running operations on `ApplicationEdit`, `CertificateRequest`, and `CertificateGenerate`. Styled by `app.css` (`.progress-log*`).
...
- Static assets live under `wwwroot/`: `app.css` (only the few rules MudBlazor doesn't cover — `.page` grid, main/article spacing, `.json-editor-wrap` Monaco sizing, `.progress-log*` — **bare-tag selectors are forbidden** because MudBlazor renders raw `input`/`fieldset`/`label`/etc. internally and a global rule will leak into its components; see `tasks/lessons.md`), `logo.svg`, `favicon.png`.
...
GET    /api/certificates/{id}/download         → 200 application/x-pem-file (chain only) | 404
...
1. `JsonApplicationRepository` and `JsonCertificateRepository` each hold a `CancellationTokenSource`. Mutations no longer auto-fire — `IRuntimeReloadService` (and the renewal workers) call `SignalReload()` explicitly, which swaps in a fresh CTS via `Interlocked.Exchange` and `Cancel()`s the old one. `GetReloadToken()` returns a `CancellationChangeToken` wrapping the current CTS.
...
3. `SniCertificateSelector` subscribes to *both* repos' reload tokens (apps decide which hostnames need a cert; certs may be uploaded/deleted). Its `Reload` re-projects the host→`X509Certificate2` map. Loaded `X509Certificate2` instances are **cached across reloads by `(certId, thumbprint)`** — if the manifest's `Thumbprint` matches the cached entry, the disk read + PEM→PFX roundtrip is skipped. When a cert is replaced (thumbprint changed) or orphaned (no app references it anymore), the prior `X509Certificate2` is **deferred-disposed 60 seconds later** so Schannel can finish in-flight handshakes against it (immediate disposal causes "Cannot find the requested object" errors on the very next handshake). Disposal scheduling uses the injected `TimeProvider` so tests can advance virtual time. The selector's `Dispose()` cancels all pending deferred disposals and disposes everything immediately. **`Select(sni)` is lock-free**: `_byHost` is a `volatile Dictionary<string, X509Certificate2>` that reload writers atomically replace; readers do one volatile load and a `TryGetValue` per TLS handshake (no lock, no allocation). The dict is treated as immutable post-publication, so concurrent readers and a publishing writer never race on contents.
4. `PassthroughRouteTable` (singleton, `HomeYarp.Application/Tls/`) subscribes to the app repo's reload token and maintains an `ImmutableDictionary<host, Application>` (+ wildcard suffix list) for O(1) `TryResolve(sni, out app)`. `TlsPassthroughConnectionHandler` consumes this instead of doing a per-connection `GetAllAsync` + nested LINQ scan.

### Snapshot-backed repositories

Both repos hold a `volatile ApplicationSnapshot` / `CertificateSnapshot` (immutable record holding `ImmutableArray<T> All` + `ImmutableDictionary<Guid, T> ById` + case-insensitive `ImmutableDictionary<string, T> ByName`). Reads are lock-free: `GetSnapshot()` is one volatile read, `GetByIdAsync`/`GetByNameAsync` wrap the snapshot's dictionary lookup in `Task.FromResult`, `GetAllAsync` returns the snapshot's `All` array. Writers acquire `_writeGate` (a `SemaphoreSlim`) only to serialize file I/O; once the file is on disk they rebuild the snapshot from the post-mutation `All` (via `ApplicationSnapshot.FromItems`) and `volatile`-store it. Because the rebuild reads from `newAll` rather than diffing against the prior snapshot, callers can mutate-in-place (Blazor JSON-edit path does this) and the `ByName` index stays correct after a rename. Initial load happens **synchronously in the constructor** — small-N JSON deserialize, fine at homelab scale, and removes a class of "consumer started before first read" races. Hot consumers (`HomeYarpConfigProvider.BuildConfig`, `SniCertificateSelector.ReloadCore`, `PassthroughRouteTable.RebuildCore`) call `GetSnapshot()` directly instead of going `.GetAwaiter().GetResult()` over the async API.
...
| `SelfSignedRenewalService` | Hosted (Singleton) | Background loop that auto-rotates self-signed certs within `RenewBefore` of expiry. |
...
- **`SelfSignedRenewalService` : `BackgroundService`** waits `SelfSigned:StartupDelay` (60s default) then loops every `SelfSigned:RenewalInterval` (24h default). Each tick it lists certs, filters those with `SelfSigned is not null && NotAfter - now < RenewBefore`, and calls `RegenerateAsync(cert.Id, ct)` per cert — same id, fresh key, fresh expiry, hostnames carried over. Failures log and continue — the next tick retries. The loop exits immediately if `SelfSigned:Enabled == false`, so flipping the flag requires a restart. Manual regeneration via UI button or `POST /api/certificates/{id}/regenerate` still works on demand. Wildcards and IP SANs are accepted (since we control issuance).

Configuration lives under `HomeYarp:SelfSigned` in `appsettings.json`:

| Key | Default | Notes |
|---|---|---|
| `Enabled` | `true` | Master switch. Default-on — self-signed renewal is purely local with no rate limits or external deps. Renewal worker exits immediately when false. |
| `RenewBefore` | `30.00:00:00` | Renew when `NotAfter - now` is below this. |
| `RenewalInterval` | `24:00:00` | How often the background worker checks. |
| `StartupDelay` | `00:01:00` | Grace period before the first tick. |
